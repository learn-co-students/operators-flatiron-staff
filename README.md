...
3. The first method, `unsafe?` will take in an argument of a speed and return "Drive better!" if the speed is unsafe and "Great driving!" if it is safe. 
  * Use an `if`/`else` statement pair to build the `unsafe?` method. It should return `"Drive better!"` if the speed is either below `40` or above `60`. *Going 30 mph on the freeway would be unsafe, as would going 95 mph. Going 50 miles per hour, however, would return `"Great driving!"` as that's within the "safe" range.*
